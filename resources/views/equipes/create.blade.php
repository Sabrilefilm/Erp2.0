...
<div class="space-y-6 pb-8 max-w-xl">
    <div class="rounded-2xl overflow-hidden bg-gradient-to-br from-[#1e3a8a] to-[#1e40af] border border-white/10 p-6">
        <h1 class="text-2xl font-bold text-white">Créer une agence</h1>
        <p class="text-blue-200/90 text-sm mt-1">Une agence correspond à une équipe dans l'application.</p>
    </div>

    <form action="{{ route('equipes.store') }}" method="POST" class="rounded-2xl border border-white/10 bg-white/[0.02] p-6 space-y-4">
        @csrf
        <div>
            <label for="nom" class="block text-sm font-medium text-[#94a3b8] mb-1">Nom de l'agence *</label>
            <input type="text" name="nom" id="nom" value="{{ old('nom') }}" required maxlength="255"
        ...
        <div>
            ...
            <select name="manager_id" id="manager_id" class="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white focus:border-sky-500 focus:ring-1 focus:ring-sky-500">
                ...
                @foreach($agentsEtManagers as $u)
                    <option value="{{ $u->id }}" {{ old('manager_id') == $u->id ? 'selected' : '' }}>{{ $u->name }} ({{ $u->getRoleLabel() }})</option>
                @endforeach
            </select>
        ...
        <div class="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4">
            <label class="flex items-start gap-3 cursor-pointer">
                ...
                <input type="checkbox" name="est_partenaire" value="1" {{ old('est_partenaire') ? 'checked' : '' }}
        ...
        <div class="flex gap-3 pt-2">
            <button type="submit" class="px-4 py-2.5 rounded-xl bg-sky-600 hover:bg-sky-500 text-white font-medium">Créer</button>
