...
@section('content')
{{-- ── Rappel rapport de la semaine (obligatoire pour tous sauf fondateurs et créateurs) ── --}}
@if(!empty($rapportVendrediManquant))
<div class="mb-4 rounded-2xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 flex items-center justify-between gap-4 flex-wrap">
    ...
    <a href="{{ route('rapport-vendredi.index') }}" class="shrink-0 px-4 py-2 rounded-xl bg-amber-500/30 hover:bg-amber-500/40 text-amber-200 font-semibold text-sm transition-colors">Remplir le rapport</a>
</div>
@endif
{{-- ── Bandeau messages non lus (disparaît en cliquant sur Messagerie) ── --}}
@php $dashUnread = \App\Models\Message::where('receiver_id', auth()->id())->whereNull('read_at')->count(); @endphp
@if($dashUnread > 0 && !request()->routeIs('messagerie.*'))
<a href="{{ route('messagerie.index') }}"
   ...
   style="background:linear-gradient(135deg,rgba(0,212,255,0.12),rgba(14,165,233,0.08));border:1px solid rgba(0,212,255,0.25);">
    <span class="relative shrink-0">
        ...
        <span style="position:absolute;top:-5px;right:-5px;min-width:16px;height:16px;background:#00d4ff;color:#0a0e27;font-size:9px;font-weight:800;border-radius:8px;display:flex;align-items:center;justify-content:center;padding:0 3px;">{{ $dashUnread > 99 ? '99+' : $dashUnread }}</span>
    </span>
    <div class="flex-1 min-w-0">
        <p class="text-sm font-semibold text-white leading-none">
            {{ $dashUnread === 1 ? 'Vous avez 1 message non lu' : "Vous avez {$dashUnread} messages non lus" }}
        </p>
...
<div class="space-y-5">
    ...
    {{-- Anniversaire : animation pour l'utilisateur dont c'est le jour — date_naissance renseignée dans le profil --}}
    ...
    <div class="rounded-2xl overflow-hidden bg-gradient-to-r from-pink-500/25 via-rose-500/20 to-amber-500/20 border border-pink-400/30 p-4 md:p-5 animate-pulse" style="animation-duration: 2s;">
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="text-lg font-bold text-white">Joyeux anniversaire, {{ auth()->user()->name }} !</p>
    ...
    {{-- ── Hero créateur : bonjour + données stream + bonnes actions (petit, fun) ── --}}
    <div class="db-hero db-fade">
        <div class="flex flex-col sm:flex-row sm:items-center gap-4">
            <div class="db-avatar">{{ $initials }}</div>
            <div class="flex-1 min-w-0">
                <p class="db-hero-greeting">Bonjour</p>
                <h1 class="db-hero-name mt-0.5">{{ auth()->user()->name }}</h1>
                <p class="db-hero-role mt-0.5">Données de stream</p>
                <div class="flex flex-wrap items-center gap-2 mt-2">
                    @if(isset($scoreFideliteMois) && $scoreFideliteMois !== null)
                    <a href="{{ route('score-fidelite.index') }}" class="db-fidelite-badge inline-flex items-center gap-1.5 no-underline group {{ $scoreFideliteMois == 0 ? 'is-zero' : '' }}">
                        <span class="db-fidelite-emoji">🎁</span>
                        @if($scoreFideliteMois == 0)
                        <span class="db-fidelite-text">0 pt ce mois</span>
                        @else
                        <span class="db-fidelite-plus db-fidelite-text">+{{ $scoreFideliteMois }} pts ce mois</span>
                        @endif
                        <span class="db-fidelite-detail text-[10px] opacity-70 group-hover:opacity-100">→ détail</span>
                    </a>
                    @endif
                    @if(isset($scoreIntegrite))
                    <a href="{{ route('score-integrite.index') }}" class="db-integrite-badge inline-flex items-center gap-1.5 no-underline group">
                        <span class="db-fidelite-emoji">📊</span>
                        <span class="db-fidelite-text">Intégrité {{ $scoreIntegrite }}/100</span>
                        <span class="db-fidelite-detail text-[10px] opacity-70 group-hover:opacity-100">→ détail</span>
                    </a>
                    @endif
                </div>
            </div>
            <a href="{{ route('formations.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#1877f2] hover:bg-[#166fe5] text-white text-sm font-semibold transition-colors shrink-0">
    ...
    {{-- Score d'intégrité 30–60 : affichage sur l'accueil (zone orange) ── --}}
    @if(isset($scoreIntegrite) && $scoreIntegrite >= 30 && $scoreIntegrite < 60)
    <a href="{{ route('score-integrite.index') }}" class="db-fade flex items-center gap-4 p-4 rounded-2xl border border-amber-500/30 bg-amber-500/10 hover:bg-amber-500/15 transition-colors no-underline group">
        <span class="w-12 h-12 rounded-xl bg-amber-500/20 flex items-center justify-center shrink-0 text-2xl">📊</span>
        <div class="flex-1 min-w-0">
            <p class="text-amber-400 font-semibold text-sm">Score d'intégrité : {{ $scoreIntegrite }}/100 (zone orange)</p>
    ...
    {{-- ── Stats stream + objectif (une seule carte compacte) ── --}}
    <div class="db-fade rounded-2xl border border-white/08 bg-white/[0.03] overflow-hidden">
        ...
        <div class="flex items-center justify-between px-4 pt-4 pb-3">
            ...
            @if($cr && $moisDisponibles && $moisDisponibles->isNotEmpty())
            <form id="form-mois-dashboard" method="get" action="{{ route('dashboard') }}" class="flex items-center gap-1.5">
                <select name="annee" class="rounded-md bg-white/5 border border-white/10 text-white text-[11px] px-2 py-1 focus:outline-none focus:border-[#00d4ff]/50">
                    ...
                    <option value="{{ $y }}" {{ (int)($annee ?? 0) === (int)$y ? 'selected' : '' }}>{{ $y }}</option>
                    @endforeach
                </select>
                <select name="mois" class="rounded-md bg-white/5 border border-white/10 text-white text-[11px] px-2 py-1 focus:outline-none focus:border-[#00d4ff]/50">
                    ...
                    <option value="{{ $m['mois'] }}" {{ (int)($mois ?? 0) === (int)$m['mois'] ? 'selected' : '' }}>{{ $m['libelle'] }}</option>
                    @endforeach
                </select>
            </form>
            <script>
        ...
        {{-- 3 stats — toujours sur une seule ligne, sans trait --}}
        <div class="flex gap-2 px-3 py-2">
            <div class="flex-1 flex flex-col items-center gap-1 py-3 px-2 text-center rounded-xl bg-white/[0.03]">
                <span class="text-base">📅</span>
                <span class="text-lg font-bold text-white leading-none">{{ isset($jours) && $jours !== null ? $jours : '—' }}</span>
                <span class="text-[10px] text-white/35 font-semibold uppercase tracking-wider">Jours</span>
            </div>
            <div class="flex-1 flex flex-col items-center gap-1 py-3 px-2 text-center rounded-xl bg-white/[0.03]">
                <span class="text-base">⏱️</span>
                <span class="text-lg font-bold text-white leading-none">{{ isset($heures) && $heures !== null ? \App\Support\HeuresHelper::format((float) $heures) : '—' }}</span>
                <span class="text-[10px] text-white/35 font-semibold uppercase tracking-wider">Heures</span>
            </div>
            <div class="flex-1 flex flex-col items-center gap-1 py-3 px-2 text-center rounded-xl bg-white/[0.03]">
                <span class="text-base">💎</span>
                <span class="text-lg font-bold text-white leading-none">{{ isset($diamants) && $diamants !== null ? number_format($diamants, 0, ',', ' ') : '—' }}</span>
                <span class="text-[10px] text-white/35 font-semibold uppercase tracking-wider">Diamants</span>
            </div>
        </div>
        {{-- Objectif compact — Jours/Heures/Diamants ci‑dessus = données import Excel, pas inventées. Code couleur : rouge <30 %, orange 30–50 %, blanc OK ≥50 % --}}
        @php
        ...
        <div class="px-4 pb-4 pt-2">
            <div class="flex items-center justify-between mb-1.5">
                <span class="text-[11px] text-white/40">{{ $objectifJours }}j - {{ $objectifHeures }}h objectif</span>
                <span class="text-sm font-bold inline-flex items-center gap-1.5" style="color: {{ $pctColor }}"><span aria-hidden="true">{{ $pctEmoji }}</span>{{ $pctGlobal }}%</span>
            </div>
            <div class="db-progress-track">
                <div class="db-progress-fill" style="width:{{ min(100,$pctGlobal) }}%; background: {{ $pctColor }};"></div>
            </div>
        </div>
    </div>

    {{-- ── Mon agent / Mon équipe (à jour depuis la fiche créateur) ── --}}
    @if($cr && ($cr->agent || $cr->equipe))
    <div class="db-encadrement db-fade">
        ...
        <div class="db-encadrement-row">
            ...
            <div>
                ...
                <div class="db-encadrement-value">{{ $cr->agent->name }}</div>
            </div>
        </div>
        @php
        ...
        <div class="db-encadrement-row whatsapp">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="db-encadrement-value">
                    @if($waUrl)
                    <a href="{{ $waUrl }}" target="_blank" rel="noopener noreferrer">{{ $cr->agent->phone }}</a>
                    @else
            ...
            @if($waUrl)
            <a href="{{ $waUrl }}" target="_blank" rel="noopener noreferrer" class="shrink-0 px-3 py-1.5 rounded-lg bg-[#25d366]/20 text-[#25d366] text-xs font-semibold hover:bg-[#25d366]/30 transition-colors">Contacter</a>
            @endif
        </div>
        @endif
        @if($cr->equipe)
        <div class="db-encadrement-row equipe">
            ...
            <div>
                ...
                <div class="db-encadrement-value">{{ $cr->equipe->nom }}</div>
            </div>
        </div>
        @endif
    </div>
    ...
    {{-- ── Accès rapide créateur — une seule ligne ── --}}
    <div class="db-fade mt-2">
        <p class="db-section-title">Accès rapide</p>
        <div class="flex gap-3">
            <a href="{{ route('matches.index') }}" class="qa-card qa-purple flex-1">
            ...
            <a href="{{ route('recompenses.index') }}" class="qa-card qa-green flex-1">
    ...
    {{-- ── Message agence + Campagne TikTok (compact) ── --}}
    <div class="db-fade mt-2 grid grid-cols-1 md:grid-cols-2 gap-3">
        <div class="db-agency-box">
            ...
            <div class="db-agency-box-content">
                @if(!empty($messageAgence))
                    <div class="font-semibold text-white/90">{{ $messageAgence->titre }}</div>
                    <div class="muted mt-1">{{ \Illuminate\Support\Str::limit(trim($messageAgence->contenu), 140) }}</div>
                @else
                    <div class="muted">Aucun message pour le moment.</div>
                @endif
            </div>
            <div class="db-agency-box-actions">
                <a class="db-mini-link" href="{{ route('regles.index') }}">Voir tous les messages</a>
            </div>
        </div>
        ...
        <div class="db-agency-box">
            ...
            <div class="db-agency-box-actions">
                <a class="db-mini-link" href="{{ route('formations.index', $hasCatalogueTiktok ? ['catalogue' => 'tiktok'] : []) }}">Ouvrir</a>
            </div>
        </div>
    </div>
</div>
...
@else
{{-- ═══════════════════════════════════════════════════
     DASHBOARD STAFF / AGENT / FONDATEUR
═══════════════════════════════════════════════════ --}}
@php
...
<div class="space-y-5">
    ...
    <div class="db-hero db-fade">
        <div class="flex flex-col sm:flex-row sm:items-center gap-4">
            ...
            <div class="flex-1 min-w-0">
                <p class="db-hero-greeting">Bonjour</p>
                <h1 class="db-hero-name mt-0.5">{{ auth()->user()->name }}</h1>
                <p class="db-hero-role mt-0.5">{{ auth()->user()->getRoleLabel() }}</p>
            </div>
            <a href="{{ route('createurs.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#1877f2] hover:bg-[#166fe5] text-white text-sm font-semibold transition-colors shrink-0">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                Créateurs
            </a>
        </div>
    ...
    {{-- ── Chiffres clés : une ligne simple ── --}}
    <div class="db-fade rounded-xl border border-white/10 bg-white/[0.03] p-4">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-xl font-bold text-white tabular-nums">{{ $createurs }}</p>
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-xl font-bold text-white tabular-nums">{{ $vuesAffichage }}</p>
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-xl font-bold text-white tabular-nums">{{ $followersAffichage }}</p>
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-xl font-bold text-white tabular-nums">{{ $equipesCount }}</p>
    ...
    <div class="db-fade mt-2">
        <p class="db-section-title">Accès rapide</p>
        {{-- Défilement horizontal sur mobile pour rester sur une seule ligne --}}
        <div class="flex gap-3 overflow-x-auto pb-1 -mx-1 px-1">
            <a href="{{ route('createurs.index') }}" class="qa-card qa-blue shrink-0 w-20">
            ...
            <a href="{{ route('matches.index') }}" class="qa-card qa-purple shrink-0 w-20">
                <div class="qa-icon"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg></div>
                <div class="qa-label">Matchs</div>
            </a>
            <a href="{{ route('recompenses.index') }}" class="qa-card qa-green shrink-0 w-20">
                <div class="qa-icon"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7"/></svg></div>
                <div class="qa-label">Récompenses</div>
            </a>
            @endif
            <a href="{{ route('messagerie.index') }}" class="qa-card qa-pink shrink-0 w-20">
            ...
            <a href="{{ route('equipes.index') }}" class="qa-card qa-blue shrink-0 w-20">
            ...
            <a href="{{ route('score-integrite.gestion') }}" class="qa-card qa-amber shrink-0 w-20">
            ...
            @endif
            <a href="{{ route('formations.index') }}" class="qa-card qa-blue shrink-0 w-20">
                <div class="qa-icon"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg></div>
                <div class="qa-label">Formations</div>
            </a>
            <a href="{{ route('regles.index') }}" class="qa-card qa-slate shrink-0 w-20">
    ...
    <div class="db-fade mt-2 grid grid-cols-1 md:grid-cols-2 gap-3">
        <div class="db-agency-box">
            ...
            <div class="db-agency-box-content">
                @if(!empty($messageAgence))
                    <div class="font-semibold text-white/90">{{ $messageAgence->titre }}</div>
                    <div class="muted mt-1">{{ \Illuminate\Support\Str::limit(trim($messageAgence->contenu), 160) }}</div>
                @else
                    <div class="muted">Aucun message pour le moment.</div>
                @endif
            </div>
            ...
            <div class="flex items-center justify-between px-4 py-3 rounded-xl border border-white/07 bg-white/[0.03] hover:bg-white/[0.06] hover:border-white/12 transition-all group">
                <div class="flex items-center gap-3 min-w-0">
                    <div class="w-8 h-8 rounded-lg bg-[#00d4ff]/15 flex items-center justify-center shrink-0">
                        <span class="text-[#00d4ff] text-xs font-bold">{{ strtoupper(mb_substr($eq->nom, 0, 2)) }}</span>
                    </div>
                    <span class="text-sm font-medium text-white truncate">{{ $eq->nom }}</span>
                </div>
                <span class="text-xs font-bold text-[#00d4ff] bg-[#00d4ff]/10 px-2 py-1 rounded-lg shrink-0 ml-2">{{ $eq->createurs_count ?? $eq->createurs()->count() }}</span>
            </div>
            @endforeach
        </div>
    </div>
