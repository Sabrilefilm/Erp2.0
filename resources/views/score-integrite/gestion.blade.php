...
@section('title', 'Gestion des infractions — Score d\'intégrité')
...
<div class="space-y-6 pb-8 max-w-4xl mx-auto">
    <div class="rounded-2xl overflow-hidden bg-gradient-to-br from-amber-500/20 via-orange-500/10 to-red-500/10 border border-white/10 p-6 md:p-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex items-center gap-3 shrink-0 flex-wrap">
                <span class="text-2xl font-bold text-white tabular-nums">{{ $scoreActuel }}<span class="text-[#94a3b8] font-normal text-lg">/{{ $scoreMax }}</span></span>
                <span class="text-xs text-[#94a3b8]">Score actuel · {{ $createurSelect->nom }}</span>
                <form action="{{ route('createurs.destroy', $createurSelect) }}" method="POST" class="inline" onsubmit="return confirm('Supprimer définitivement ce créateur ({{ $createurSelect->nom }}) et toutes ses données (score, historique, sanctions, etc.) ? Cette action est irréversible.');">
                    @csrf
                    @method('DELETE')
    ...
    <div class="rounded-2xl border border-white/10 overflow-hidden bg-white/5">
        ...
        <form action="{{ route('score-integrite.store-infraction') }}" method="POST" class="p-5 space-y-4" id="form-infraction">
            @csrf
            @php
            ...
            <div class="relative" x-data="{
                createurs: {{ Js::from($createursSiJson) }},
                query: {{ Js::from($initialSiCreateur ? $initialSiCreateur->nom . ($initialSiCreateur->pseudo_tiktok ? ' (@' . $initialSiCreateur->pseudo_tiktok . ')' : '') : '') }},
                selectedId: {{ Js::from((string)$initialSiId) }},
                open: false,
                focusedIndex: 0,
                gestionUrl: {{ Js::from(route('score-integrite.gestion')) }},
                get filtered() {
            ...
            <div>
                ...
                <textarea name="details_infraction" id="details_infraction" rows="3" required class="ultra-input w-full px-4 py-3 rounded-xl text-white text-sm border border-white/10 bg-white/5 placeholder-[#64748b]" placeholder="Décrire l'infraction ou la raison de la modification du score...">{{ old('details_infraction') }}</textarea>
                @error('details_infraction')
                <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                @enderror
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="score_consequent" id="score_consequent" value="{{ old('score_consequent', $scoreActuel) }}" min="0" max="100" required class="ultra-input w-full px-4 py-3 rounded-xl text-white text-sm border border-white/10 bg-white/5 tabular-nums">
                    <p class="mt-1 text-xs text-[#64748b]">Score actuel : {{ $scoreActuel }}. Baisser pour une infraction, augmenter pour une régularisation.</p>
                    @error('score_consequent')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    <label for="sanction_infraction" class="block text-sm font-medium text-[#94a3b8] mb-1">Sanction (optionnel)</label>
                    <input type="text" name="sanction_infraction" id="sanction_infraction" value="{{ old('sanction_infraction') }}" class="ultra-input w-full px-4 py-3 rounded-xl text-white text-sm border border-white/10 bg-white/5" placeholder="Ex. Avertissement, mise en garde...">
                    @error('sanction_infraction')
                    <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full min-w-[640px] text-sm">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-white/10 hover:bg-white/5 transition-colors {{ $loop->even ? 'bg-white/[0.03]' : '' }}">
                        <td class="py-3.5 px-4 font-medium text-white">{{ $h->createur?->nom ?? '—' }}</td>
                        <td class="py-3.5 px-4 whitespace-nowrap text-[#b0bee3]">{{ $h->heure_modification?->translatedFormat('d/m/Y H:i') ?? '—' }}</td>
                        <td class="py-3.5 px-4 text-white/95 max-w-xs">{{ Str::limit($h->details_infraction, 60) }}</td>
                        <td class="py-3.5 px-4 tabular-nums text-center text-[#94a3b8]">{{ $h->score_avant }}</td>
                        <td class="py-3.5 px-4 tabular-nums text-center font-semibold text-white">{{ $h->score_consequent }}</td>
                        <td class="py-3.5 px-4 text-white/90">{{ $h->sanction_infraction ?? '—' }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
