...
<div class="max-w-5xl mx-auto px-2 sm:px-4 pt-6 pb-12" x-data="formationModal()" @keydown.escape.window="closeModal()">
    ...
    <template x-if="open">
        <div class="fmodal-backdrop" @click.self="closeModal()">
            <div class="fmodal">
                ...
                {{-- Bandeau héro coloré avec image YouTube si dispo --}}
                ...
                {{-- Lecteur vidéo OU placeholder design --}}
                ...
                <div class="fmodal-body">
                    ...
                    {{-- Actions : quiz, document à télécharger, ouvrir ressource, accéder au contenu --}}
    ...
    <div class="flex items-start justify-between mb-8">
        ...
        <a href="{{ route('formations.create') }}" title="Ajouter un module" class="mt-1 w-9 h-9 rounded-xl flex items-center justify-center bg-white/5 hover:bg-white/10 border border-white/10 text-[#94a3b8] hover:text-white transition-all">
    ...
    <div class="filter-bar">
        ...
        <div class="filter-row mb-3">
            <a href="{{ route('formations.index', array_filter(['q' => request('q'), 'catalogue' => request('catalogue')])) }}" class="filter-pill {{ !request('type') ? 'active' : '' }}">Tous</a>
            <a href="{{ route('formations.index', array_filter(['type' => 'video', 'q' => request('q'), 'catalogue' => request('catalogue')])) }}" class="filter-pill {{ request('type') === 'video' ? 'active' : '' }}">
            ...
            <a href="{{ route('formations.index', ['type' => 'document', 'q' => request('q'), 'catalogue' => request('catalogue')]) }}" class="filter-pill {{ request('type') === 'document' ? 'active' : '' }}">
            ...
            <a href="{{ route('formations.index', ['type' => 'lien', 'q' => request('q'), 'catalogue' => request('catalogue')]) }}" class="filter-pill {{ request('type') === 'lien' ? 'active' : '' }}">
            ...
            <a href="{{ route('formations.index', array_filter(['q' => request('q'), 'type' => request('type')])) }}" class="filter-pill {{ !request('catalogue') ? 'active' : '' }}">Tous les thèmes</a>
            @foreach($catalogues as $cat)
            <a href="{{ route('formations.index', ['catalogue' => $cat->slug, 'q' => request('q'), 'type' => request('type')]) }}" class="filter-pill {{ request('catalogue') === $cat->slug ? 'active' : '' }}">{{ $cat->label }}</a>
            @endforeach
        </div>
        <form action="{{ route('formations.index') }}" method="GET" class="search-wrap">
            @if(request('type'))<input type="hidden" name="type" value="{{ request('type') }}">@endif
            @if(request('catalogue'))<input type="hidden" name="catalogue" value="{{ request('catalogue') }}">@endif
            ...
            <input type="text" name="q" value="{{ request('q') }}" placeholder="Rechercher un module…" class="search-input">
            <button type="submit" class="search-btn">Rechercher</button>
            @if(request('q') || request('type') || request('catalogue'))
            <a href="{{ route('formations.index') }}" class="text-xs text-[#94a3b8] hover:text-white transition-colors">Réinitialiser</a>
            @endif
        </form>
    </div>

    @if(session('success'))
    <div class="rounded-xl bg-[#00d4ff]/10 border border-[#00d4ff]/30 text-[#00d4ff] text-sm px-4 py-3 mb-6">{{ session('success') }}</div>
    @endif

    {{-- Titre section --}}
    <div class="flex items-center justify-between mb-5">
        <h2 class="text-xl font-bold text-white">
            @if(request('q'))Résultats pour « {{ request('q') }} »
            @elseif(request('catalogue')){{ $catalogues->firstWhere('slug', request('catalogue'))?->label ?? request('catalogue') }}
            @elseif(request('type') && isset(\App\Models\Formation::TYPES[request('type')])){{ \App\Models\Formation::TYPES[request('type')] }}s
            @else À la une @endif
        </h2>
        @if(request('catalogue') || request('type') || request('q'))
        <a href="{{ route('formations.index') }}" class="text-sm font-semibold text-[#00d4ff] bg-[#00d4ff]/10 px-4 py-1.5 rounded-lg hover:bg-[#00d4ff]/20 transition-colors">Tout afficher</a>
        @endif
    </div>
    ...
    <div class="rounded-2xl border border-white/10 bg-white/[0.03] p-12 text-center">
        ...
        @if(auth()->user()->canAddEntries())
        <a href="{{ route('formations.create') }}" class="inline-block mt-4 px-4 py-2 rounded-xl bg-[#00d4ff] text-[#0a0e27] text-sm font-semibold hover:bg-[#00d4ff]/90 transition-colors">Ajouter un module</a>
        @endif
    </div>
    @else
    <div class="formation-grid">
        ...
        @endphp
        <div class="explore-card {{ $f->actif ? '' : 'opacity-60' }}"
             @click="openModal({
                titre:        {{ Js::from($f->titre) }},
                description:  {{ Js::from($f->description) }},
                descriptionHtml: {{ Js::from($descHtml) }},
                catalogue:    {{ Js::from($catalogueLabel) }},
                typeLabel:    {{ Js::from($contentLabel) }},
                type:         {{ Js::from($f->type) }},
                youtubeId:    {{ Js::from($youtubeId) }},
                thumb:        {{ Js::from($thumb) }},
                grad:         {{ Js::from($grad) }},
                gradLight:    {{ Js::from($gradLight) }},
                hasQuiz:      {{ $f->questions_count > 0 ? 'true' : 'false' }},
                quizUrl:      '{{ route('formations.quiz.show', $f) }}',
                externalUrl:  {{ Js::from($f->type !== 'video' && $f->url ? $f->url : null) }},
                hasFichier:   {{ $f->fichier_path ? 'true' : 'false' }},
                fichierUrl:   {{ Js::from($f->fichier_path ? route('formations.fichier', $f) : null) }},
                fichierNom:   {{ Js::from($f->fichier_nom ?? '') }},
                contenuUrl:   '{{ route('formations.contenu', $f) }}',
                isAgent:      {{ $isAgent ? 'true' : 'false' }},
                editUrl:      '{{ route('formations.edit', $f) }}'
             })">

            <div class="explore-card-banner" style="background:{{ $grad }};">
                @if($thumb)
                <img src="{{ $thumb }}" alt="" loading="lazy" onerror="this.style.display='none'">
                @endif
                <div class="explore-card-banner-content">
                    <p class="explore-card-category">{{ $contentLabel }}</p>
                    <p class="explore-card-title">{{ $f->titre }}</p>
                </div>
                <div class="explore-play-btn">
                    @if($f->type==='video')<svg fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
                    @elseif($f->type==='document')<svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                    @else<svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>@endif
                </div>
            </div>
            <div class="explore-card-footer">
                <div class="explore-stat">
                    <span class="explore-stat-value" style="font-size:13px;">{{ $f->type==='video'?'🎬':($f->type==='document'?'📄':'🔗') }}</span>
                    <span class="explore-stat-label">{{ $typeLabel }}</span>
                </div>
                <div class="explore-stat-divider"></div>
                <div class="explore-stat">
                    <span class="explore-stat-value" style="font-size:12px;">{{ $f->questions_count > 0 ? '✅' : '—' }}</span>
            ...
            <div class="card-agent-row" @click.stop="">
                <a href="{{ route('formations.edit', $f) }}" class="text-xs text-[#94a3b8] hover:text-white flex items-center gap-1 transition-colors">
                ...
                <form action="{{ route('formations.destroy', $f) }}" method="POST" class="inline ml-auto" onsubmit="return confirm('Supprimer ce module ?');">
                    @csrf @method('DELETE')
