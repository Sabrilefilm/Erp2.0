<?php $__env->startSection('title', 'Import Excel'); ?>
...
<div class="max-w-4xl space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <a href="<?php echo e(route('import.corriger-heures-jours')); ?>" class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/40 text-amber-200 text-sm font-medium shrink-0">
    ...
    <div id="import-result-container" class="space-y-4" style="<?php echo e($hasResult ? '' : 'display:none'); ?>">
        
        <div id="import-result-bandeau">
        <?php if($displayLog && is_array($displayLog)): ?>
        <div class="rounded-xl border border-white/10 overflow-hidden">
            <div class="flex flex-wrap items-center gap-4 px-5 py-4 bg-[#0d1117]">
                ...
                <span class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-500/20 border border-emerald-500/40 text-emerald-400 font-semibold">
                    <span aria-hidden="true">✓</span> <?php echo e($countOk); ?> ligne(s) OK
                </span>
                <?php if($countErr > 0): ?>
                <span class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-red-500/20 border border-red-500/40 text-red-400 font-semibold">
                    <span aria-hidden="true">✗</span> <?php echo e($countErr); ?> erreur(s)
                </span>
                <?php endif; ?>
                <?php if(isset($showLog) && $showLog): ?>
                <span class="text-white/50 text-sm ml-auto"><?php echo e($showLog->created_at->format('d/m/Y H:i')); ?> · <?php echo e($showLog->fichier); ?></span>
                <?php endif; ?>
            </div>
            
            <?php if($countErr > 0 && count($errorsAndTips) > 0): ?>
            <div class="px-5 py-4 border-t border-white/10 bg-red-950/20">
                ...
                <ul class="space-y-2 font-mono text-[13px]">
                    <?php $__currentLoopData = $errorsAndTips; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $e): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php $type = $e['type'] ?? ''; $msg = $e['msg'] ?? ''; $line = $e['line'] ?? 0; ?>
                    <li class="flex gap-3 <?php echo e($type === 'error' ? 'text-red-400' : 'text-[#00d4ff] pl-4 border-l-2 border-[#00d4ff]/50'); ?>">
                        <?php if($line > 0): ?><span class="shrink-0 text-white/40">Ligne <?php echo e($line); ?></span><?php endif; ?>
                        <span class="min-w-0"><?php echo e($msg); ?></span>
                    </li>
        ...
        <div class="rounded-xl border border-white/10 bg-[#0a0e17] overflow-hidden shadow-2xl shadow-black/40">
            <div class="flex items-center justify-between gap-3 px-4 py-3 border-b border-white/10 bg-[#161b22]">
                ...
                <div id="import-terminal-meta" class="font-mono text-xs text-white/50">
                    <?php if(isset($showLog) && $showLog): ?>
                    <?php echo e($showLog->created_at->format('d/m/Y H:i:s')); ?> · <?php echo e($showLog->fichier); ?>
            ...
            <div id="import-summary" class="import-summary border-b border-white/10 bg-[#0d1117]/80 px-4 py-3 font-mono text-[13px]">
                ...
                <div class="text-emerald-400">✓ <?php echo e(session('success')); ?></div>
                <?php endif; ?>
                <?php if(session('warning')): ?>
                <div class="text-amber-400">⚠ <?php echo e(session('warning')); ?></div>
                <?php endif; ?>
                <?php if(session('error')): ?>
                <div class="text-red-400">✗ <?php echo e(session('error')); ?></div>
                <?php if(session('import_solution')): ?>
                <div class="mt-2 text-[#7dd3fc] pl-4 border-l-2 border-[#00d4ff]/50">💡 <?php echo e(session('import_solution')); ?></div>
                <?php endif; ?>
                <?php endif; ?>
                <?php if(isset($showLog) && $showLog && $showLog->message && !session('success') && !session('warning') && !session('error')): ?>
                <div class="<?php echo e($showLog->statut === 'succes' ? 'text-emerald-400' : ($showLog->statut === 'partiel' ? 'text-amber-400' : 'text-red-400')); ?>"><?php echo e($showLog->statut === 'succes' ? '✓' : ($showLog->statut === 'partiel' ? '⚠' : '✗')); ?> <?php echo e($showLog->message); ?></div>
                <?php endif; ?>
            </div>
            <?php $statsFromLog = isset($showLog) && $showLog ? sprintf('Fichier: %s  |  Lignes importées: %s  |  Erreurs: %s', $showLog->fichier, $showLog->lignes_importees, $showLog->lignes_erreur) : null; ?>
            <div id="import-stats-block" class="border-b border-white/10 bg-[#161b22]/60 px-4 py-2.5 font-mono text-[12px] text-white/70 <?php echo e($statsFromLog ? '' : 'hidden'); ?>">
                <span id="import-stats-text"><?php echo e($statsFromLog ?? '—'); ?></span>
            </div>
            <div id="import-terminal-wrapper" class="<?php echo e(($displayLog && is_array($displayLog)) ? '' : 'hidden'); ?>">
                ...
                <div id="import-terminal" class="p-4 font-mono text-[13px] leading-[1.6] max-h-[60vh] overflow-y-auto overflow-x-auto whitespace-pre-wrap break-words" style="min-height: 180px;">
                    <?php if($displayLog && is_array($displayLog)): ?>
                    <?php $__currentLoopData = $displayLog; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $entry): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php
                    ...
                    <div class="flex gap-3 py-0.5 border-b border-white/[0.03] last:border-0 import-log-row
                        <?php echo e($type === 'success' ? 'text-emerald-400' : ''); ?>

                        <?php echo e($type === 'error' ? 'text-red-400' : ''); ?>

                        <?php echo e($type === 'solution' ? 'text-[#00d4ff] bg-[#00d4ff]/05 -mx-1 px-2 py-1 rounded' : ''); ?>

                        <?php echo e($type === 'info' ? 'text-sky-300/90' : ''); ?>" data-type="<?php echo e($type); ?>">
                        <span class="shrink-0 text-white/25 select-none w-6 text-right"><?php echo e($line > 0 ? $line : ''); ?></span>
                        <span class="shrink-0 text-white/40 select-none w-14 text-right"><?php echo e($prefix); ?></span>
                        <span class="min-w-0"><?php echo e($msg); ?></span>
                    </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    <?php endif; ?>
    ...
    <div class="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-4">
        <form id="import-form" action="<?php echo e(route('import.store')); ?>" method="POST" enctype="multipart/form-data" class="space-y-4">
            <?php echo csrf_field(); ?>
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="mois" id="mois" class="w-full px-3 py-2 rounded-lg bg-gray-800 border border-gray-700 text-white">
                        <?php $__currentLoopData = ['01' => 'Janvier', '02' => 'Février', '03' => 'Mars', '04' => 'Avril', '05' => 'Mai', '06' => 'Juin', '07' => 'Juillet', '08' => 'Août', '09' => 'Septembre', '10' => 'Octobre', '11' => 'Novembre', '12' => 'Décembre']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $num => $nom): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($num); ?>" <?php echo e($num == now()->format('m') ? 'selected' : ''); ?>><?php echo e($nom); ?></option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
                </div>
            </div>
...
$message = $__bag->first($__errorArgs[0]); ?><p class="mt-1 text-sm text-accent-red"><?php echo e($message); ?></p><?php unset($message);
            ...
            <table class="w-full min-w-[500px]">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-800/50">
                        <td class="p-3 text-sm"><?php echo e($log->created_at->format('d/m/Y H:i')); ?></td>
                        <td class="p-3"><?php echo e($log->user->name); ?></td>
                        <td class="p-3 text-sm"><?php echo e($log->fichier); ?></td>
                        <td class="p-3">
                            <?php if($log->statut === 'succes'): ?>
                            <span class="text-green-400">Succès</span>
                            <?php elseif($log->statut === 'partiel'): ?>
                        ...
                        <td class="p-3 text-right">
                            <?php echo e($log->lignes_importees); ?> / <?php echo e($log->lignes_importees + $log->lignes_erreur); ?>

                            <?php if($log->log_detail && count($log->log_detail) > 0): ?>
                            <a href="<?php echo e(route('import.index', ['show_log' => $log->id])); ?>" class="ml-1 text-[11px] text-[#00d4ff] hover:underline">Voir le journal</a>
                            <?php endif; ?>
                        </td>
                        <td class="p-3 text-center">
                            <form action="<?php echo e(route('import.logs.destroy', $log)); ?>" method="post" class="inline" onsubmit="return confirm('Supprimer cette entrée de l\'historique ?');">
                                <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/sabri/Documents/ERP Unions V2/resources/views/import/index.blade.php ENDPATH**/ ?>