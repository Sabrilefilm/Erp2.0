...
<div class="match-page min-h-screen pb-24 md:pb-12">
    <header class="match-hero sticky top-0 z-10 border-b border-white/10 bg-[#0b0f1a]/90 backdrop-blur-md md:static">
        <div class="max-w-3xl mx-auto px-4 md:px-6 py-4 flex items-center justify-between gap-3">
            ...
            <?php if(auth()->user()->canProgrammerMatch()): ?>
            <a href="<?php echo e(route('matches.create')); ?>" class="inline-flex items-center gap-2 rounded-xl bg-cyan-500 px-4 py-2.5 text-sm font-medium text-white shadow-lg shadow-cyan-500/25 transition hover:bg-cyan-400 hover:shadow-cyan-500/30 focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:ring-offset-2 focus:ring-offset-[#0b0f1a]">
    ...
    <main class="match-main max-w-3xl mx-auto px-4 md:px-6 py-6 space-y-5">
        ...
        <div class="match-section rounded-xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 text-sm px-4 py-3">✓ <?php echo e(session('success')); ?></div>
        <?php endif; ?>
        <?php if(session('error')): ?>
        <div class="match-section rounded-xl bg-red-500/15 border border-red-500/30 text-red-400 text-sm px-4 py-3"><?php echo e(session('error')); ?></div>
        <?php endif; ?>

        
        <?php if(($vue ?? '') !== 'aujourdhui' && isset($matchsAujourdhuiEtDemain) && $matchsAujourdhuiEtDemain->isNotEmpty()): ?>
        <section class="match-section rounded-xl border border-cyan-500/20 bg-cyan-500/5 p-4">
            ...
            <ul class="space-y-1.5">
                <?php $__currentLoopData = $matchsAujourdhuiEtDemain; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ma): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php
                ...
                ?>
                <li class="rounded-lg bg-white/[0.03] border border-white/5 text-xs overflow-hidden <?php echo e($bientotLabel ? 'ring-1 ring-amber-400/30' : ''); ?>">
                    <div class="flex flex-wrap items-center gap-x-2 gap-y-0.5 py-1.5 px-2">
                        <span class="px-1.5 py-0.5 rounded text-[10px] font-medium <?php echo e($estDemain ? 'bg-cyan-500/25 text-cyan-300' : 'bg-emerald-500/25 text-emerald-300'); ?>"><?php echo e($jourLabel); ?></span>
                        <?php if($bientotLabel): ?><span class="match-bientot-badge <?php echo e($bientotClass); ?>">⏱ <?php echo e($bientotLabel); ?></span><?php endif; ?>
                        <span class="tabular-nums text-[#94a3b8]"><?php echo e($ma->heure ? substr($ma->heure, 0, 5) : '—'); ?></span>
                        <span class="px-1.5 py-0.5 rounded bg-cyan-500/20 text-cyan-300"><?php echo e($ma->type === 'match_off' ? ($ma->avec_boost ? 'Match officiel avec boost' : 'Match officiel sans boost') . ($ma->niveau_match ? ' · ' . (\App\Models\Planning::NIVEAUX_MATCH_OFF[$ma->niveau_match] ?? $ma->niveau_match) : '') : ($typeLabels[$ma->type] ?? $ma->type)); ?></span>
                        <span class="text-white truncate max-w-[120px]"><?php echo e($ma->createur?->nom ?? '—'); ?></span>
                        <span class="vs-animated">Vs</span>
                        <span class="text-cyan-300 truncate max-w-[100px]"><?php if($ma->createur_adverse_at): ?><?php echo e(str_starts_with($ma->createur_adverse_at, '@') ? $ma->createur_adverse_at : '@'.$ma->createur_adverse_at); ?><?php else: ?>—<?php endif; ?></span>
                        <?php if($ma->createur && $ma->createur->equipe): ?><span class="text-[10px] text-[#64748b]">(<?php echo e($ma->createur->equipe->nom); ?>)</span><?php endif; ?>
                        <?php if($sm === 'en_cours'): ?><span class="ml-auto px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-400">En cours</span><?php endif; ?>
                    </div>
                    <?php if(auth()->user()->isCreateur()): ?>
                    <div class="px-2 pb-1.5 pt-0 border-t border-white/5 text-[10px] text-[#64748b]">
                        Affiché 7/7 24/24 <?php if($ma->creePar): ?> · Par <?php echo e($ma->creePar->name); ?><?php endif; ?>
                        <?php if($ma->updated_par && $ma->updated_at): ?><span class="text-amber-400/90"> · Modifié le <?php echo e($ma->updated_at->translatedFormat('d/m/Y')); ?></span><?php endif; ?>
                    </div>
                    <?php endif; ?>
                </li>
        ...
        <?php if(auth()->user()->isCreateur() && $prochainsMatchs->isNotEmpty()): ?>
        <section class="match-section rounded-xl border border-white/10 bg-white/5 p-4">
            ...
            <div class="space-y-1.5">
                <?php $__currentLoopData = $prochainsMatchs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $prochain): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php
                ...
                ?>
                <div class="rounded-lg bg-white/[0.03] border border-white/5 overflow-hidden <?php echo e($bientotProchain ? 'ring-1 ring-amber-400/30' : ''); ?>">
                    <div class="flex flex-wrap items-center gap-x-2 text-xs py-1.5 px-2">
                        <span class="font-medium text-cyan-400"><?php echo e($jLabel); ?></span>
                        <?php if($bientotProchain): ?><span class="match-bientot-badge <?php echo e($bientotProchainClass); ?>">⏱ <?php echo e($bientotProchain); ?></span><?php endif; ?>
                        <span class="text-[#94a3b8]">·</span>
                        <span class="text-white"><?php echo e($prochain->type === 'match_off' ? ($prochain->avec_boost ? 'Match officiel avec boost' : 'Match officiel sans boost') . ($prochain->niveau_match ? ' · ' . (\App\Models\Planning::NIVEAUX_MATCH_OFF[$prochain->niveau_match] ?? $prochain->niveau_match) : '') : ($typeLabels[$prochain->type] ?? $prochain->type)); ?></span>
                        <?php if($prochain->heure): ?><span class="text-white"><?php echo e(substr($prochain->heure, 0, 5)); ?></span><?php endif; ?>
                        <span class="text-white"><?php echo e($prochain->createur?->nom ?? '—'); ?></span>
                        <span class="text-[#94a3b8]">vs</span>
                        <?php if($prochain->createur_adverse_at): ?><span class="text-cyan-300"><?php echo e(str_starts_with($prochain->createur_adverse_at, '@') ? $prochain->createur_adverse_at : '@'.$prochain->createur_adverse_at); ?></span><?php else: ?><span class="text-[#64748b]">—</span><?php endif; ?>
                    </div>
                    <div class="px-2 pb-1.5 pt-0 border-t border-white/5 text-[10px] text-[#64748b] space-y-0.5">
                        <?php if($prochain->createur_adverse || $prochain->createur_adverse_agent): ?>
                        <div class="text-[#94a3b8]">
                            <?php if($prochain->createur_adverse): ?><span class="text-white/90"><?php echo e($prochain->createur_adverse); ?></span><?php endif; ?>
                            <?php if($prochain->createur_adverse_agent): ?><?php if($prochain->createur_adverse): ?> · <?php endif; ?><span>Agent <?php echo e($prochain->createur_adverse_agent); ?></span><?php endif; ?>
                        </div>
                        <?php endif; ?>
                        Affiché 7/7 24/24 <?php if($prochain->creePar): ?> · Par <?php echo e($prochain->creePar->name); ?><?php endif; ?>
                        <?php if($prochain->updated_par && $prochain->updated_at): ?><span class="text-amber-400/90"> · Modifié le <?php echo e($prochain->updated_at->translatedFormat('d/m/Y')); ?></span><?php endif; ?>
                    </div>
                </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
        ...
        <nav class="match-section match-tabs flex flex-wrap items-center gap-1 border-b border-white/10 -mx-4 px-4 md:mx-0 md:px-0 pb-2" role="tablist">
            <a href="<?php echo e($urlAujourdhui); ?>" class="px-4 py-2.5 text-sm font-medium rounded-t-lg border-b-2 <?php echo e(($vue ?? '') === 'aujourdhui' ? 'border-amber-400 text-amber-400' : 'border-transparent text-slate-400 hover:text-white'); ?>">Aujourd'hui</a>
            <a href="<?php echo e($urlUnions); ?>" class="px-4 py-2.5 text-sm font-medium rounded-t-lg border-b-2 <?php echo e(($vue ?? '') === 'unions' ? 'border-red-400 text-red-400' : 'border-transparent text-slate-400 hover:text-white'); ?>">Match Unions</a>
            <a href="<?php echo e($urlPartenaire); ?>" class="px-4 py-2.5 text-sm font-medium rounded-t-lg border-b-2 <?php echo e(($vue ?? '') === 'partenaire' ? 'border-cyan-400 text-cyan-400' : 'border-transparent text-slate-400 hover:text-white'); ?>">Match partenaire</a>
            <?php if(!empty($showCatalogueEquipeSelect) && ($vue ?? '') === 'partenaire'): ?>
            <form action="<?php echo e(route('matches.index')); ?>" method="GET" class="ml-auto flex items-center gap-2">
                <?php $__currentLoopData = request()->only(['vue', 'from', 'to', 'type', 'createur_id', 'statut']); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k => $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                ...
                <select name="equipe_id" onchange="this.form.submit()" class="ultra-input px-2 py-1.5 rounded-lg text-white text-xs border-0 bg-white/5 min-w-[120px]">
                    ...
                    <option value="<?php echo e($eq->id); ?>" <?php echo e((string)($filterEquipeId ?? '') === (string)$eq->id ? 'selected' : ''); ?>><?php echo e($eq->nom); ?></option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
            </form>
            <?php endif; ?>
        ...
        <section class="match-section rounded-xl border border-amber-500/25 bg-amber-500/5 p-4" aria-labelledby="demandes-titre">
            ...
            <ul class="space-y-1.5">
                ...
                <li class="flex flex-wrap items-center justify-between gap-x-2 gap-y-1 py-1.5 px-2 rounded-lg bg-white/5 border border-white/5 text-xs">
                    <span class="font-medium text-white"><?php echo e($d->createur?->nom ?? '—'); ?></span>
                    <span class="text-[#94a3b8]"><?php echo e($d->date_souhaitee->format('d/m')); ?> · <?php echo e($typeLabels[$d->type] ?? $d->type); ?></span>
                    <?php if($d->qui_en_face): ?><span class="text-[#64748b]">vs <?php echo e($d->qui_en_face); ?></span><?php endif; ?>
                    <div class="ml-auto flex gap-1">
                        <a href="<?php echo e(route('matches.create', $params)); ?>" class="ultra-btn-cta px-2 py-1 text-[11px]"><span>Accepter</span></a>
                        <a href="<?php echo e(route('matches.create', $params)); ?>" class="px-2 py-1 rounded bg-white/10 text-[#94a3b8] text-[11px]">Modifier</a>
                        <form action="<?php echo e(route('matches.demande.refuse', $d)); ?>" method="POST" class="inline" onsubmit="return confirm('Refuser cette demande ?');"><?php echo csrf_field(); ?>
        ...
        <details class="match-section match-filters-block rounded-xl border border-white/10 bg-white/5 overflow-hidden" <?php if(request()->hasAny(['from','to','type','createur_id','statut','vue','equipe_id'])): ?> open <?php endif; ?>>
            <summary class="px-4 py-3 cursor-pointer list-none flex items-center justify-between gap-3 hover:bg-white/5 transition-colors">
                ...
                <span class="text-slate-400 text-xs tabular-nums"><?php echo e(\Carbon\Carbon::parse($from)->format('d/m')); ?> → <?php echo e(\Carbon\Carbon::parse($to)->format('d/m')); ?></span>
            ...
            <form action="<?php echo e(route('matches.index')); ?>" method="GET" class="p-4 pt-2 grid grid-cols-2 md:flex md:flex-wrap gap-3 border-t border-white/10 bg-black/20">
                <?php if(!empty($showTabsVue) && isset($vue) && $vue !== ''): ?><input type="hidden" name="vue" value="<?php echo e($vue); ?>"><?php endif; ?>
                <?php if(!empty($filterEquipeId)): ?><input type="hidden" name="equipe_id" value="<?php echo e($filterEquipeId); ?>"><?php endif; ?>
                <div class="min-w-[120px]"><label class="block text-xs font-medium text-slate-300 mb-1">Du</label><input type="date" name="from" value="<?php echo e($from); ?>" class="ultra-input w-full px-3 py-2 rounded-lg text-white text-sm border border-white/10 bg-white/5"></div>
                <div class="min-w-[120px]"><label class="block text-xs font-medium text-slate-300 mb-1">Au</label><input type="date" name="to" value="<?php echo e($to); ?>" class="ultra-input w-full px-3 py-2 rounded-lg text-white text-sm border border-white/10 bg-white/5"></div>
                <div class="min-w-[140px]"><label class="block text-xs font-medium text-slate-300 mb-1">Type</label><select name="type" class="ultra-input w-full px-3 py-2 rounded-lg text-white text-sm border border-white/10 bg-white/5"><option value="">Tous</option><?php $__currentLoopData = $typeLabels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $v => $l): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($v); ?>" <?php if($filterType === $v): ?> selected <?php endif; ?>><?php echo e($l); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></select></div>
                <?php if(!auth()->user()->isCreateur()): ?>
                <div class="min-w-[160px]"><label class="block text-xs font-medium text-slate-300 mb-1">Créateur</label><select name="createur_id" class="ultra-input w-full px-3 py-2 rounded-lg text-white text-sm border border-white/10 bg-white/5"><option value="">Tous</option><?php $__currentLoopData = $createurs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($c->id); ?>" <?php if((string)$filterCreateurId === (string)$c->id): ?> selected <?php endif; ?>><?php echo e($c->nom); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></select></div>
                <?php endif; ?>
                <div class="min-w-[120px]"><label class="block text-xs font-medium text-slate-300 mb-1">Statut</label><select name="statut" class="ultra-input w-full px-3 py-2 rounded-lg text-white text-sm border border-white/10 bg-white/5"><option value="">Tous</option><?php $__currentLoopData = $statutLabels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $v => $l): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($v); ?>" <?php if($filterStatut === $v): ?> selected <?php endif; ?>><?php echo e($l); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></select></div>
        ...
        <div class="flex flex-wrap items-center justify-between gap-2">
            ...
            <p class="text-xs text-[#94a3b8] flex items-center gap-2 flex-wrap">
                <?php if($matchs->total() > 0): ?>
                <span class="font-semibold text-white"><?php echo e($matchs->total()); ?></span> match<?php echo e($matchs->total() > 1 ? 's' : ''); ?>

                <?php else: ?>
                Aucun match
                <?php endif; ?>
                <a href="<?php echo e(route('matches.pdf', request()->query())); ?>" target="_blank" rel="noopener" class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-red-500/20 text-red-400 hover:bg-red-500/30 text-xs font-medium transition-colors">
        ...
        <?php if(($vue ?? '') === 'aujourdhui'): ?>
        <p class="text-xs font-semibold text-white/70 mb-2">Matchs à venir — 7 jours (2 agences)</p>
        <?php elseif(isset($matchsAujourdhuiEtDemain) && $matchsAujourdhuiEtDemain->isNotEmpty()): ?>
        <p class="text-xs font-semibold text-white/70 mb-2">Autres matchs (hors aujourd'hui et demain)</p>
        <?php endif; ?>
        <?php if($matchs->isEmpty()): ?>
        <div class="match-empty rounded-xl border border-white/10 bg-white/5 p-10 text-center <?php echo e(($vue ?? '') === 'partenaire' ? 'border-cyan-500/20 bg-cyan-500/5' : ''); ?>">
            <?php if(($vue ?? '') === 'partenaire'): ?>
            ...
            <?php if(isset($matchsAujourdhuiEtDemain) && $matchsAujourdhuiEtDemain->isNotEmpty()): ?>
            ...
            <?php else: ?>
            <p class="text-slate-400 text-sm">Aucun match (<?php echo e(\Carbon\Carbon::parse($from)->translatedFormat('d M')); ?> → <?php echo e(\Carbon\Carbon::parse($to)->translatedFormat('d M Y')); ?>).</p>
            <?php endif; ?>
        ...
        <ul class="space-y-4 match-list">
            <?php $__currentLoopData = $matchs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $m): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php
            ...
            ?>
            <li class="match-card rounded-2xl border border-white/10 bg-white/[0.06] overflow-hidden relative <?php echo e($bientotCard ? 'ring-1 ring-amber-400/40' : ''); ?>">
                
                <div class="pt-4 px-4 pb-1">
                    <span class="inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-medium
                        <?php if($s === 'en_cours'): ?> bg-amber-500/25 text-amber-400
                        <?php elseif($s === 'acceptee'): ?> bg-emerald-500/25 text-emerald-400
                        <?php elseif($s === 'manque'): ?> bg-red-500/25 text-red-400
                        <?php elseif($s === 'refusee'): ?> bg-orange-500/25 text-orange-400
                        <?php else: ?> bg-slate-500/25 text-slate-300
                        <?php endif; ?>">
                        <?php if($s === 'en_cours'): ?>
                            ...
                            En cours
                        <?php elseif($s === 'acceptee'): ?> OK
                        <?php elseif($s === 'manque'): ?> Manqué
                        <?php elseif($s === 'refusee'): ?> Refusée
                        <?php else: ?> Programmé
                        <?php endif; ?>
                    </span>
                    <?php if($bientotCard): ?><span class="match-bientot-badge ml-2 <?php echo e($bientotCardClass); ?>">⏱ <?php echo e($bientotCard); ?></span><?php endif; ?>
                    <?php if($m->updated_par): ?><span class="ml-2 inline-flex rounded-full px-2 py-0.5 text-[10px] bg-amber-500/20 text-amber-400">Modif</span><?php endif; ?>
                </div>
                
                <div class="px-4 pb-2">
                    <h3 class="text-base sm:text-lg font-semibold text-white leading-tight truncate" title="<?php echo e($m->createur?->nom ?? '—'); ?> Vs <?php echo e($adverseAt); ?>">
                        <span class="text-white"><?php echo e($m->createur?->nom ?? '—'); ?></span>
                        <span class="vs-animated mx-1">Vs</span>
                        <span class="text-cyan-400"><?php echo e($adverseAt); ?></span>
                    </h3>
                </div>
                ...
                <div class="px-4 pb-4">
                    <div class="flex flex-wrap items-center gap-2">
                        <?php if(auth()->user()->canProgrammerMatch()): ?>
                        <a href="<?php echo e(route('matches.edit', $m)); ?>" class="ultra-btn-cta inline-flex items-center justify-center text-sm px-4 py-2.5"><span>Modifier</span></a>
                        <?php endif; ?>
                        <?php if((auth()->user()->isFondateur() || auth()->user()->isManageur()) && ($m->createur_adverse_numero || $m->createur_adverse_at || $m->createur_adverse || $m->createur_adverse_agent || $m->createur_adverse_email || $m->createur_adverse_agence || $m->createur_adverse_autres)): ?>
                        <button type="button" class="inline-flex items-center gap-1.5 rounded-xl border border-cyan-500/40 bg-cyan-500/10 text-cyan-400 hover:bg-cyan-500/20 text-sm px-3 py-2 js-open-infos-adverse"
                                data-at="<?php echo e($m->createur_adverse_at ?? ''); ?>"
                                data-nom-agence="<?php echo e($m->createur_adverse ?? ''); ?>"
                                data-agence="<?php echo e($m->createur_adverse_agence ?? ''); ?>"
                                data-agent="<?php echo e($m->createur_adverse_agent ?? ''); ?>"
                                data-numero="<?php echo e($m->createur_adverse_numero ?? ''); ?>"
                                data-email="<?php echo e($m->createur_adverse_email ?? ''); ?>"
                                data-autres="<?php echo e($m->createur_adverse_autres ?? ''); ?>" title="Voir les infos adverse">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            Infos
                        </button>
                        <?php endif; ?>
                        <?php if(auth()->user()->canProgrammerMatch()): ?>
                        <form action="<?php echo e(route('matches.destroy', $m)); ?>" method="POST" class="inline" onsubmit="return confirm('Supprimer ce match ?');"><?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                ...
                <div class="flex flex-wrap items-center gap-x-4 gap-y-1 px-4 py-3 border-t border-white/5 bg-black/10 text-xs text-slate-400">
                    <span class="inline-flex items-center gap-1.5">
                        ...
                        <time class="tabular-nums"><?php echo e($m->date->translatedFormat('d/m')); ?><?php if($m->heure): ?> <?php echo e(substr($m->heure, 0, 5)); ?><?php else: ?> —<?php endif; ?></time>
                    </span>
                    <span class="inline-flex items-center gap-1.5">
                    ...
                    <span class="inline-flex items-center gap-1.5">
                        ...
                        <span><?php echo e($m->createur->equipe->nom); ?></span>
                    </span>
                    <?php endif; ?>
                </div>
                ...
                <div class="px-4 py-2 border-t border-white/5 text-[10px] text-slate-500">
                    Affiché 7/7 24/24
                    <?php if($m->creePar): ?>· Par <?php echo e($m->creePar->name); ?><?php endif; ?>
                    <?php if($m->updated_par && $m->updated_at): ?><span class="text-amber-400/90"> · Modifié le <?php echo e($m->updated_at->translatedFormat('d/m/Y')); ?></span><?php endif; ?>
                </div>
                <?php endif; ?>
            </li>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        ...
        <?php if($matchs->hasPages()): ?>
        <div class="flex justify-center py-4"><?php echo e($matchs->links()); ?></div>
        <?php endif; ?>
        <?php endif; ?>

        
        <?php if($matchs->total() > 0 && $matchs->hasPages()): ?>
        <p class="text-center">
            <a href="<?php echo e(route('matches.index', array_merge(request()->query(), ['from' => now()->startOfYear()->format('Y-m-d'), 'to' => now()->format('Y-m-d')]))); ?>" class="text-xs text-cyan-400 hover:text-cyan-300 font-medium">Voir tout l'historique</a>
        </p>
        <?php endif; ?>
        ...
        <?php if(auth()->user()->isCreateur()): ?>
        <div class="pt-4 pb-4">
            <a href="<?php echo e(route('matches.demande.create')); ?>" class="ultra-btn-cta btn-demande-match">
...
<?php if(auth()->user()->isFondateur() || auth()->user()->isManageur()): ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/sabri/Documents/ERP Unions V2/resources/views/matches/index.blade.php ENDPATH**/ ?>